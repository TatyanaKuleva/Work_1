...
def favorable_categories_of_increased_cashback(data, year, month):
    ...
    service_logger.info(f"определяет дату начала периода")
    date_start = datetime.datetime(year = year_dt.year, month=month_dt.month, day=1)
    service_logger.info(f"определяет дату окончания периода")
    ...
    service_logger.info(f"фильтрует операции относящиеся к заданному периоду от {date_start} до {date_end}")
    ...
    service_logger.info(f"формирует датафрейм из списка транзакций")
    df_filtr_list_transaction = pd.DataFrame(filtr_list_transaction)

    service_logger.info(f"группирует транзакции по категориям и сортируер по сумме кэшбека")
    ...
    service_logger.info(f"формирует словарь результатов")
    res_dict = dict()
    for index, row in sort_for_sum_cashback.iterrows():
        res_dict[row["Категория"]] = row["Кэшбэк"]

    service_logger.info(f"формирует Json ответ")
    json_result = json.dumps(res_dict, indent=4,  ensure_ascii=False)

    return json_result

def investment_bank(month, transactions, limit):
    """возвращает сумму, которую удалось бы отложить в «Инвесткопилку» через настриваемый предел округления трат"""
    ...
    service_logger.info(f"филььтрует транзакции относящиеся к расходам")
    df_only_expenses = df_filtr_list_transaction.loc[df_filtr_list_transaction['Сумма операции'] < 0].copy()

    service_logger.info(f"вычисляет сумму округления по каждой транзакции до заданного лимита")
    ...
    service_logger.info(f"вычисляет сумму окгрулений за период")
...
def basic_search(data_dict:list[dict], str_for_search:str):
    ...
    result_list = []
    service_logger.info(f"формирует шаблон для поиска по заданной строке запроса")
    pattern = re.compile(str_for_search, flags=re.IGNORECASE)

    service_logger.info(f"осуществляет поиск и формирует список транзакций")
...
def phone_search(data_dict:list[dict]):
    ...
    result_list = []
    service_logger.info(f"формирует шаблон для поиска телефонного номера в строке")
...
def search_transfer_for_individuals(data_dict:list[dict]):
    ...
    result_list = []
    service_logger.info(f"формирует шаблон для отбора категории Переводы")
    pattern_cat = re.compile(r'перев', flags=re.IGNORECASE)
    service_logger.info(f"формирует шаблон для отбора переводов фл")
...
if __name__ == '__main__':
    print(search_transfer_for_individuals(df))
